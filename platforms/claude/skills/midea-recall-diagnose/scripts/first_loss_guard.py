...
def main() -> int:
    ...
    doc_path_first_loss = None
    if args.target_type == "DOC":
        doc_path_order = [cmp_id for cmp_id in order if cmp_id in DOC_ORDER]
        for cmp_id in doc_path_order:
            if stage_state.get(cmp_id) == "false":
                doc_path_first_loss = cmp_id
                break
    ...
    if args.target_type == "DOC" and doc_path_first_loss:
        print(f"DOC_PATH_FIRST_LOSS={doc_path_first_loss}")
    print(f"ORDER: {order}")
    for cmp_id in order:
