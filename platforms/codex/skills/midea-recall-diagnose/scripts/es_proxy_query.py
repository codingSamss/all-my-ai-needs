#!/usr/bin/env python3
"""Query Elasticsearch through the cloud console proxy without Playwright."""
...
def resolve_browser_name(cli_browser: Optional[str], env_cfg: Dict[str, Any]) -> str:
    es_console = ensure_dict(env_cfg.get("es_console"), "env.es_console")
...
def build_session(
    ...
    *,
    ...
    if not hostname:
        raise CliError(f"Invalid request_proxy_url: {proxy_url}")
...
def resolve_proxy_url(route: Dict[str, Any]) -> str:
    proxy_url = clean_str(route.get("request_proxy_url"))
    if not proxy_url:
        raise CliError("esConsoleRoute.request_proxy_url is required")
    if "<" in proxy_url or ">" in proxy_url:
        raise CliError(f"request_proxy_url still contains placeholder: {proxy_url}")
    return proxy_url
...
def request_headers(proxy_url: str, route: Dict[str, Any]) -> Dict[str, str]:
    parsed = urlsplit(proxy_url)
...
def query_es_proxy(
    ...
    headers = request_headers(proxy_url, route)
    payload = {
        "cinsId": cins_id,
        "path": path,
        "method": method,
...
def main() -> int:
    ...
    try:
        ...
        proxy_url = resolve_proxy_url(route)
        cins_id = resolve_cins_id(route)
