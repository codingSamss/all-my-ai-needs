...
- 引用：路径使用反引号，引用单行行号格式 `file:42`。
- 默认只保留路径倒数 1-2 级 + 行号（如 `SearchServiceAdapter.java:133` 或 `adapter/SearchServiceAdapter.java:133`）；代码与文档场景一致执行。
- 仅在存在同名冲突或定位歧义时，才扩展到更长的相对路径；避免长绝对路径。
