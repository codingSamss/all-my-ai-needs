...
cleanup_legacy_taste_skills() {
  local target_root="$1"
  local skill_name=""
  local target_dir=""

  # Taste 已收敛为 taste-design 单入口；清理旧入口，避免运行时继续暴露 13 个平铺 skill。
  for skill_name in "${legacy_taste_skill_dirs[@]}"; do
    target_dir="$target_root/$skill_name"
    [ -e "$target_dir" ] || continue
    if [ "$DRY_RUN" = "true" ]; then
      echo "[清理预览] 删除旧 Taste 运行入口: $target_dir"
    else
      rm -rf "$target_dir"
      echo "[清理] 删除旧 Taste 运行入口: $target_dir"
    fi
  done
}
...
if [ "$SYNC_SKILLS" = "true" ]; then
  ...
  cleanup_legacy_taste_skills "$CODEX_HOME_DIR/skills"
